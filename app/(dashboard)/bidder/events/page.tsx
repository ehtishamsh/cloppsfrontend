...
import { bidderService, type AvailableEvent } from "@/services/bidder"
...
export default function EventsPage() {
  ...
  const loadEvents = async () => {
    try {
      const data = await bidderService.getAvailableEvents()
      setEvents(data)
    } catch (error) {
  ...
  const handleRequestEnrollment = async (eventId: string) => {
    ...
    try {
      await bidderService.requestEnrollment(eventId)
  ...
  const handleInvitationResponse = async (eventId: string, accept: boolean) => {
    try {
      await bidderService.respondToInvitation(eventId, accept)
